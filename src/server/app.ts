...
/**
 * The server-side shape of a product profile. The core consumes only these
 * values; a profile package (src/profile/*) supplies them. The core never
 * imports a profile module.
 */
export interface ServerProfile {
  readonly id: string;
  readonly name: string;
  /** Developer-controlled settings defaults; user overrides win at runtime. */
  readonly settingsDefaults: Record<string, string>;
  /** Property-geography columns, in display order (CSV + address join). */
  readonly geoColumns: readonly string[];
}

/**
 * Settings every market shares. The profile's defaults are merged over these
 * so a missing row still resolves to a locale and a due day.
 */
const CORE_DEFAULT_SETTINGS: Record<string, string> = {
  default_rent_due_day: "1",
  late_fee_amount: "50",
  late_fee_grace_days: "5",
};

/** Build the property input schema with the profile's geo columns appended. */
function buildPropertyInput(geoColumns: readonly string[]): z.ZodObject<z.ZodRawShape> {
  const geoShape: z.ZodRawShape = {};
  for (const col of geoColumns) geoShape[col] = z.string().optional().nullable();
  return z.object({
    name: z.string().min(1),
...
// whole build — so the defaults are written here, once, when the settings
// table is still empty. A re-deploy never resurrects a row the user deleted.
//
// The sample portfolio (Austin fixture) is NOT part of the commercial runtime:
// it lives as a dev/QA fixture only (tests/helpers/demo-fixture.ts) and the
// app itself never seeds it.

let DEFAULT_SETTINGS: Record<string, string> = CORE_DEFAULT_SETTINGS;
...
async function ensureSeeded(): Promise<void> {
  ...
  try {
    ...
    // yet: the next request retries. Never fail a request over defaults.
    seeded = false;
  }
  resetAuthCache();
}
...
let PropertyInput: ReturnType<typeof buildPropertyInput> = buildPropertyInput([]);
...
app.post("/api/properties", async (c) => {
  ...
  for (const key of ["country", ...GEO_COLUMNS] as const) {
    const value = d[key];
...
app.get("/api/export/properties", async (c) => {
  ...
  const headers = ["id", "name", "type", "address", ...GEO_COLUMNS, "country", "city", "state", "zip", "unit_count", "occupied_count"];
  const data = rows.map((r) => {
    ...
    for (const col of GEO_COLUMNS) row[col] = (r as Record<string, unknown>)[col];
    return row;
  });
...
// ── Composition ────────────────────────────────────────────────────
// The core stays market-neutral: createApp is the only seam the deployment
// entry uses. The worker isolate runs exactly one composed app, so the
// profile-scoped values below are module singletons configured once at
// startup; integration tests exercise the single composed app too.
let GEO_COLUMNS: readonly string[] = [];

export function createApp(profile: ServerProfile): Hono<Env> {
  DEFAULT_SETTINGS = { ...CORE_DEFAULT_SETTINGS, ...profile.settingsDefaults };
  GEO_COLUMNS = profile.geoColumns;
  PropertyInput = buildPropertyInput(GEO_COLUMNS);
  return app;
}
